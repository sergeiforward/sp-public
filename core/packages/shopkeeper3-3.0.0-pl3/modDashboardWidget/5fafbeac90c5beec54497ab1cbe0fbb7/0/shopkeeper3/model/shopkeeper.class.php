...
class Shopkeeper {
    ...
    public function getCartContent()
    {
        ...
        if( !empty( $this->data ) ){
            ...
            $chunkArr = array(
                ...
                'plural' => $this->getPlural( self::$items_total, $this->lang['shk3.plural'] ),
        ...
        $output = $OnSHKbeforeCartLoad . $output;
    ...
    public function getProductsList( $purchasesData = array(), $chunkName = '' ){
        ...
        if( !empty( $purchasesData ) ){
            ...
            foreach( $purchasesData as $i => $purchase ){
                ...
                $purchase['price_total'] = $this->config['excepDigitGroup'] ? $this->numberFormat( $purchase['price'] + $additPrice ) : $purchase['price'] + $additPrice;
                $purchase['price_count'] = $this->config['excepDigitGroup'] ? $this->numberFormat( $purchase['price'] * $purchase['count'] ) : $purchase['price'] * $purchase['count'];
                $purchase['price_count_total'] = $this->config['excepDigitGroup'] ? $this->numberFormat( ( $purchase['price'] + $additPrice ) * $purchase['count'] ) : ( $purchase['price'] + $additPrice ) * $purchase['count'];
                $purchase['url_del_item'] = $this_page_url . $url_qs . "shk_action=remove&amp;n=" . $i;
                $purchase['currency'] = $this->config['currency'];
                
                $output .= $this->parseTpl( $mainChunk, $purchase, $chunkName );
    ...
    public function getProduct( $purchaseArray = array(), $packageData = array() ){
        ...
        $new_price = $this->getFromEvent(
            'OnSHKgetProductPrice',
            array( 'price' => $p_price, 'id' => $this->purchase->id, 'purchaseArray' => $this->purchase->toArray() ),
            'float'
        );
    ...
    public function getProductPrice(){
        ...
        $price = floatval( str_replace( array(',',' '), array('.',''), strval( $price ) ) );
    ...
    public function getProductOptions( $purchaseArray = array() ){
        ...
        foreach($purchaseArray as $key => $value){
            ...
            if(!empty($a_string) && $a_string=='add'){
                ...
                $output[ 'shk_' . $a_fieldname ] = array( $value, 0 );
                continue;
    ...
    public function getFromEvent( $event_name, $data = array(), $outtype = 'string' ){
        ...
        if( $outtype == 'link' ){
            ...
            $this->modx->invokeEvent( $event_name, array( $key => &$object ) );
            
            $output = $object->content;
            
        }else{
            
            $evtOut = $this->modx->invokeEvent( $event_name, $data );
            
            if( empty( $evtOut ) ) { return false; }
    ...
    public function getProductsData( $only_from_bd = false ){
        ...
        foreach( $this->data as $key => $purchase ){
            ...
            if( isset( $data[$purchase['className']][$purchase['id']] ) ){
                
                $output[$key] = array_merge( $purchase, $data[$purchase['className']][$purchase['id']] );
    ...
    public function getPurchasesOptionsData( $options ){
        ...
        if( !empty( $options ) ){
            ...
            foreach( $options as $name => $opt ){
                ...
                $full_name = $opt[0];
                if( !empty( $opt[1] ) ){
                    $full_name .= ' (' . ( $this->config['excepDigitGroup'] ? $this->numberFormat( $opt[1] ) : $opt[1] ) . ')';
                }
     ...
     *
     */
    public function getOrderData( $purchasesArray = array() ){
        
        if( empty( $purchasesArray ) ) $purchasesArray = $this->data;
        
        $output = $this->getProductsList( $purchasesArray, $this->config['orderDataRowTpl'] );
        $output = $this->stripModxTags( $output );
    ...
    public function updateDelivery( $delivery_name = false, $update_price = false ){
        ...
        if( $delivery_name !== false ){
            ...
            if( !empty( $config[ 'delivery' ] ) ){
                ...
                $new_price = $this->getFromEvent(
                    'OnSHKgetDeliveryPrice',
                    array( 'price' => $this->delivery['price'] ),
                    'float'
                );
