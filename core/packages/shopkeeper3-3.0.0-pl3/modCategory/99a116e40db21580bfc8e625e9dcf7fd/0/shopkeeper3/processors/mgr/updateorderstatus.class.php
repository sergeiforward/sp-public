    ...
    public function notifyBuyers( $order_ids, $status ){
        ...
        foreach( $order_ids as $order_id ){
            ...
            if ($response->isError()) {
                ...
                //Формируем письмо
                $orderOutputData = $shopCart->getOrderData( $purchasesArray );
                
                $contactsRowChunkName = $this->modx->getOption( 'shk3.mail_contacts_row_tpl', null, 'mailContactsRow' );
                $contactsRowChunk = $shopCart->getChunk( $contactsRowChunkName );
                $contactsOutput = $shopCart->parseTpl( $contactsRowChunk, $order_data['contacts'], $contactsRowChunkName );
